...
static int is_c_reserved(const char *name) {
    for (int i = 0; c_reserved[i] != NULL; i++) {
        if (strcmp(name, c_reserved[i]) == 0) return 1;
    }
    return 0;
}
...
static void codegen_expr(ASTNode *node) {
    ...
    switch (node->type) {
        case NODE_INT_LITERAL:
            emit_raw("%d", node->data.int_literal.value);
            break;
        ...
        case NODE_STRING_LITERAL:
            emit_raw("\"%s\"", node->data.string_literal.value);
            break;
            
        case NODE_BOOL_LITERAL:
            emit_raw("%d", node->data.bool_literal.value);
            break;
        ...
        case NODE_PIPE:
            ...
            if (node->data.pipe.right->type == NODE_MATCH) {
                ...
                // ({ int __match_N = left; int __result_N; if (...) ... __result_N; })
                int match_id = temp_counter++;
                emit_raw("({ int __match_%d = ", match_id);
                codegen_expr(node->data.pipe.left);
                emit_raw("; int __result_%d; ", match_id);
            ...
            // If right is a wand call, pipe left as first argument
                ...
                const char *fname = mangle_func_name(node->data.pipe.right->data.wand_call.name);
                emit_raw("%s(", fname);
                codegen_expr(node->data.pipe.left);
                if (node->data.pipe.right->data.wand_call.args) {
                    for (size_t i = 0; i < node->data.pipe.right->data.wand_call.args->count; i++) {
                        emit_raw(", ");
            ...
            // If right is a lambda, apply it to left
                ...
                // Inline the lambda: ({ int param = left; body; })
                ...
                if (lambda->data.lambda.params && lambda->data.lambda.params->count > 0) {
                    ...
                    emit_raw("int %s = ", param->data.param.name);
                    codegen_expr(node->data.pipe.left);
                    emit_raw("; ");
        ...
        case NODE_OBJECT:
            ...
            emit_raw("ds_object_create(%zu", node->data.object.fields ? node->data.object.fields->count : 0);
...
static void codegen_stmt(ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case NODE_VAR_DECL:
            ...
            if (node->data.var_decl.init && node->data.var_decl.init->type == NODE_ARRAY) {
                emit("int %s[1024] = ", node->data.var_decl.name);
                codegen_expr(node->data.var_decl.init);
                emit_raw(";\n");
            } else if (node->data.var_decl.init && node->data.var_decl.init->type == NODE_FLOAT_LITERAL) {
                emit("double %s = ", node->data.var_decl.name);
                codegen_expr(node->data.var_decl.init);
                emit_raw(";\n");
            } else if (node->data.var_decl.init && node->data.var_decl.init->type == NODE_STRING_LITERAL) {
                emit("const char *%s = ", node->data.var_decl.name);
                codegen_expr(node->data.var_decl.init);
                emit_raw(";\n");
            } else if (node->data.var_decl.init && node->data.var_decl.init->type == NODE_OBJECT) {
                emit("DSObject %s = ", node->data.var_decl.name);
                codegen_expr(node->data.var_decl.init);
                emit_raw(";\n");
            } else {
                emit("int %s = ", node->data.var_decl.name);
                codegen_expr(node->data.var_decl.init);
                emit_raw(";\n");
            }
            break;
        ...
        case NODE_FOR:
            ...
            if (node->data.for_loop.iterable->type == NODE_RANGE) {
                ...
                emit("for (int %s = ", node->data.for_loop.var_name);
...
static int function_has_return(ASTNode *body) {
    if (!body) return 0;
    if (body->type == NODE_RETURN && body->data.return_stmt.value) return 1;
    if (body->type == NODE_WHEN_STMT) {
        if (body->data.when_stmt.action->type == NODE_RETURN &&
            body->data.when_stmt.action->data.return_stmt.value) return 1;
    }
    if (body->type == NODE_BLOCK && body->data.block.statements) {
        for (size_t i = 0; i < body->data.block.statements->count; i++) {
            if (function_has_return(body->data.block.statements->items[i])) return 1;
        }
    }
    return 0;
}
...
static void emit_lambdas(void) {
    for (int i = 0; i < collected_lambda_count; i++) {
        ...
        emit("static int __lambda_%d(", lambda->id);
        ...
        if (lambda->body->type == NODE_BLOCK) {
            ...
            if (lambda->body->data.block.statements) {
                for (size_t j = 0; j < lambda->body->data.block.statements->count; j++) {
